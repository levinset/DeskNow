...
import { useGetAllBookingForDesks } from "../hooks/useGetAllBookingForDesks";
import DeskGuid from "../components/DeskGuid";
import { useFixRequest } from "../hooks/useFixRequest";
import { useGetUserProfile } from "../hooks/useGetUserProfile";
import { useGetOfficebyId } from "../hooks/useGetOfficebyId";
...
export default function OfficePage() {
  ...
  const handleBooking = () => {
    if (
      ...
      const maxDuration = 7 * 24 * 60 * 60 * 1000; // Maximum duration: 7 days

      if (startDay >= 1 && endDay <= 5 && duration <= maxDuration) {
        // Check if booking spans over the weekend and duration is within 7 days
        const isoStartTime = startTime.toISOString();
        const isoEndTime = endTime.toISOString();

        const deskData = {
          dateStart: `${startDate.toString().split("T")[0]}T${
            isoStartTime.split("T")[1]
          }`,
          dateEnd: `${endDate.toString().split("T")[0]}T${
            isoEndTime.split("T")[1]
          }`,
          desk: selectedDeskId,
        };
        ...
        <div className="flex flex-row gap-4 max-sm:flex-col-reverse ">
          ...
          <div className=" shadow-lg bg-[#EEF1FC] h-fit rounded-lg p-4 w-2/3 flex flex-col justify-between max-sm:w-full ">
            <div className="flex flex-col gap-1">
              ...
              <div className="hidden mt-2 max-sm:flex">
                <TimePicker
                  startTime={startTime}
                  endTime={endTime}
                  onStartTimeChange={handleStartTimeChange}
