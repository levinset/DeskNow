...
export default function DeskCard(props: DeskProps) {
  ...
  return (
    ...
    >
      ...
      <div
        id="chair"
        className={` h-16 w-6  bg-[#9D7E68]   p-2 flex flex-col justify-center items-center rounded-l-[3px]  ${
          props.showEquipment ? "block" : "hidden"
        } max-sm:h-[2rem] max-sm:w-4  `}
      >
      ...
      <div
        className={`w-3 h-20 rounded-lg rounded-l-full bg-[#9D7E68]  max-sm:h-[2.5rem] max-sm:w-2 `}
      ></div>
    </div>
  );
}
